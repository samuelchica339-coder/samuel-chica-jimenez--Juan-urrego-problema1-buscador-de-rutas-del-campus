...
Al ejecutar el programa, el usuario se encuentra con un menú principal que ofrece ocho opciones numeradas del cero al ocho. La opción más relevante es la número siete, que permite calcular la ruta más corta entre dos ubicaciones utilizando el algoritmo de Dijkstra. Para utilizarla, el usuario debe ingresar el número de la ubicación de inicio y el número de la ubicación de destino. El programa entonces calcula la ruta óptima y muestra el camino completo con las distancias acumuladas.

Para agregar una nueva ubicación, el usuario debe seleccionar la opción tres e ingresar un número identificador para la nueva ubicación. El programa agrega el nodo con una lista de adyacencia vacía y guarda automáticamente los cambios en el archivo JSON. Para agregar una ruta entre dos ubicaciones existentes, se selecciona la opción cinco y se ingresan los números de origen y destino junto con el peso deseado. El programa agrega la conexión en ambos sentidos respetando la naturaleza no dirigida del grafo.

La opción dos permite visualizar la matriz de adyacencia completa del grafo, mostrando todos los nodos ordenados numéricamente y los pesos de las conexiones entre ellos. Esta representación es útil para comprender la estructura completa del campus. La opción uno muestra la lista de adyacencia, que es más compacta y eficiente para visualizar las conexiones de cada nodo individualmente.
